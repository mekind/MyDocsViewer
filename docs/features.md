...
### 진입점 (둘 다 같은 모달, 같은 동작)
- **⌘K** (macOS) / **Ctrl+K** — 어디서든 모달 토글
- **사이드바 상단 입력창** — 포커스/타이핑하면 같은 모달이 뜨고 입력값이 전달됨

### 매칭
파일명 fuzzy. `feat` → `features.md`, `arch` → `architecture.md`. 점수화 규칙:
- 쿼리 글자가 파일명에 등장 순서대로 모두 포함되어야 매칭
- 연속 매칭(+8), 단어 경계(/, -, _, ., 공백, 첫 글자) 매칭(+5)
- 짧은 이름 가산점

검색 범위는 **파일명(basename)만**. 결과 행에는 폴더 경로가 회색으로 부가 표시되고, 매칭된 글자는 액센트 컬러로 하이라이트된다.
...
### 동작
- 결과 클릭/Enter는 기존 트리 클릭과 동일 — `location.hash` 변경.
- 결과는 점수 내림차순, 최대 50개. 빈 쿼리는 첫 50개를 그대로 보여줌.
- 인덱스는 `/api/tree` 결과를 평면화한 클라이언트 메모리. 새 파일은 ↻ Refresh로 재인덱싱된다.
